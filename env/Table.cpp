...
void Table::print(ofstream *ofs, ofstream *ofl) {
    if (!printed) {
        ...
        (*ofs) << "\treturn 0;" << endl <<
               ...
               "\t" << name << "_buffer = calloc(" << name << "_buffer_info.size, sizeof(struct " << name << "));"
               << endl <<
               "}" << endl <<
               "" << endl <<
               ...
               "\t\t" << name << "_buffer = realloc(" << name << "_buffer, " << name
               << "_buffer_info.size * sizeof(struct " << name << "*));" << endl <<
               "\t}" << endl <<
        ...
        (*ofs) << "uint64_t " << name << "_write(FILE* file) {" << endl <<
               "\tqsort(" << name << "_buffer, " << name << "_buffer_info.count, sizeof(struct " << name << "*), "
               << name << "_sort_compare);" << endl <<
               "\tfor (uint64_t i = 0; i < " << name << "_buffer_info.count; i++) {" << endl <<
        ...
        (*ofs) << "void " << name << "_index_load(struct " << Environment::get_instance()->get_name()
               << "_handle* handle) {" << endl <<
               "\tif (handle->header->count[" << name << "_header_count] == 0) {" << endl <<
               "\t\thandle->" << name << "_root = NULL;" << endl <<
               "\t\treturn;" << endl <<
               "\t}" << endl <<
               "\tint32_t levels = log(handle->header->count[" << name << "_header_count]) / log(" << name
               << "_CHILDREN) + 1;" << endl <<
               ...
               "\t\tuint64_t current_level_count = (handle->header->count[" << name << "_header_count] + pow(" << name
               << "_CHILDREN, level) - 1) / pow(" << name << "_CHILDREN, level);" << endl <<
               "\t\tcurrent_level = calloc(current_level_count, sizeof(struct " << name << "_node*));" << endl <<
               endl <<
               "\t\tfor (uint64_t i = 0; i < current_level_count; i++) {" << endl <<
               "\t\t\tfseek(handle->file, handle->header->index_offset[" << name
               << "_header_count] + (count++) * sizeof(struct " << name << "_tree_item), SEEK_SET);" << endl <<
               "\t\t\tstruct " << name << "_tree_item* current_tree_item = malloc(sizeof(struct " << name
               << "_tree_item));" << endl <<
               "\t\t\tuint64_t size = fread(current_tree_item, sizeof(struct " << name
               << "_tree_item), 1, handle->file);  if (size == 0) return;" << endl <<
               "\t\t\tcurrent_level[i] = malloc(sizeof(struct " << name << "_node));" << endl <<
               "\t\t\tmemcpy(&(current_level[i]->data), current_tree_item, sizeof(struct " << name << "_tree_item));"
               << endl <<
               ...
               "\t\t\t}" << endl <<
               endl <<
               "\t\t\tcurrent_level[i]->childs = calloc(" << name << "_CHILDREN, sizeof(struct " << name << "_node*));"
               << endl <<
               "\t\t\tuint64_t j;" << endl <<
