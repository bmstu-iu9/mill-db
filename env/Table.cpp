...
void Table::print(std::ofstream* ofs, std::ofstream* ofl) {
	...
	for (auto it = this->cols.begin(); it != this->cols.end(); it++) {
		...
		if (col->get_pk()) {
			...
			(*ofs) << indent << "if (s1->" << col->get_name() << " > s2->" << col->get_name() << ") {" << endl
			       ...
			       << indent << "} else if (s1->" << col->get_name() << " < s2->" << col->get_name() << ") {" << endl
			       << indent << "\t" << "return -1;" << endl
			       << indent << "} else {" << endl
	...
	(*ofs) << "struct " << name << "_Node* " << name << "_Node_new(bool is_leaf) {" << endl
	       << "\t" << "struct " << name << "_Node* " << name << "_Node_temp = malloc(sizeof(struct " << name << "_Node));" << endl
	       << "\t" << name << "_Node_temp->keys = malloc((2*NODE_SIZE-1) * sizeof(struct " << name << "_struct));" << endl
	...
	(*ofs) << "void " << name << "_split_child(struct " << name << "_Node* this, int i, struct " << name << "_Node* y) {" << endl
	...
	(*ofs) << "void " << name << "_insert_routine(struct " << name << "_Node* this, struct " << name << "_struct* k) {" << endl
	       << "\t" << "int i;" << endl
