...
void InsertStatement::print(ofstream *ofs, ofstream *ofl, string proc_name) {
    ...
    for (int i = 0, j = 0; i < this->get_table()->cols_size(); i++) {
        Column *col = this->get_table()->cols_at(i);
        if (col->get_type()->get_typecode() == DataType::CHAR) {
            (*ofs) << "\t" << "memcpy(inserted->" << col->get_name() << ", "
                   << this->args[j]->print() << ", "
                   << to_string(col->get_type()->get_length()) << ");" << endl;
            j++;
        } else if (col->get_type()->get_typecode() == DataType::TEXT) {
            (*ofs) << "\tsize_t " << col->get_name() << "_len = strlen(" << this->args[j]->print() << ");\n"
                << "\tinserted->" << col->get_name() << " = calloc(" << col->get_name() << "_len + 1, sizeof(char));\n"
                << "\t" << "strcpy(inserted->" << col->get_name() << ", " << this->args[j]->print() << ");\n";
            j++;
        } else {
            string arg;
            ...
            (*ofs) << "\t" << "inserted->" << col->get_name() << " = " << arg << ";" << endl;
        }
    }

    if (this->get_table()->have_text_field) {
        (*ofs) << "\t" << this->get_table()->get_name()
               << "_buffer_full_size += sizeof(struct " << this->get_table()->get_name() << "_for_storage)";

        for (auto it = this->get_table()->cols.begin(); it != this->get_table()->cols.end(); it++) {
            Column *col = *it;
            if (col->get_type()->get_typecode() == DataType::TEXT) {
                (*ofs) << " + " << col->get_name() << "_len";
            }
        }
