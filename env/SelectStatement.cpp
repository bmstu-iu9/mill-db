...
void SelectStatement::print(ofstream *ofs, ofstream *ofl, string func_name) {
    ...
    (*ofs) << "\tstruct " << Environment::get_instance()->get_name() << "_handle* handle = iter->service.handle;"
           << endl;
    (*ofs) << "\tstruct " << func_name << "_out_data* inserted = malloc(sizeof(struct " << func_name << "_out_data));"
           << endl;
    string tab = "";
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////
    for (int index = 0; index < this->tb_ind.size(); index++, tab.append("\t\t\t")) {
        ...
        (*ofs) << tab << "\tfseek(handle->file, offset, SEEK_SET);\n\n"
               << tab << "\tfor (int i = 0;; i++) {\n";

        if (table->have_text_field) {
            (*ofs) << tab << "\t\t\tstruct " << table_name << "_for_storage buffer_item;\n"
                << tab << "\t\t\tuint64_t size = fread(&buffer_item, sizeof(struct " << table_name << "_for_storage), 1, handle->file);\n"
                << tab << "\t\t\tif (size == 0) return;\n\n"
                << tab << "\t\t\tstruct " << table_name << " item = {\n";

            for (auto it = table->cols.begin(); it != table->cols.end(); it++) {
                Column *col = *it;
                char comma_or_nothing = it + 1 == table->cols.end() ? ' ' : ',';
                if (col->get_type()->get_typecode() == DataType::TEXT) {
                    (*ofs) << tab << "\t\t\t\tcalloc(buffer_item." << col->get_name() << "_len + 1, sizeof(char))" << comma_or_nothing << endl;
                } else {
                    (*ofs) << tab << "\t\t\t\tbuffer_item." << col->get_name() << comma_or_nothing << endl;
                }
            }
            ...
            for (Column *col : table->cols) {
                if (col->get_type()->get_typecode() == DataType::TEXT) {
                    (*ofs) << tab << "\t\t\tsize = fread(item." << col->get_name() << ", sizeof(char), buffer_item." << col->get_name() << "_len, handle->file);\n"
                        << tab << "\t\t\tif (size == 0) return;\n\n";
                }
            }
        } else {
            (*ofs) << tab << "\t\t\tstruct " << table_name << " item;\n"
                   << tab << "\t\t\tuint64_t size = fread(&item, sizeof(struct " << table_name << "), 1, handle->file); if (size == 0) return;\n\n";
        }
        ...
        for (Selection *sel:this->selects[tb_ind[table_name]].second) {
            (*ofs) << tab << "\t\t\t"
                   ...
                   << " item." << sel->get_column()->get_name() << ";" << endl;
        }
        for (Column *col: table->cols) {
            string s = col->get_type()->str_column_for_select(col->get_name());
            (*ofs) << tab << "\t\t\t" << s << "=" << " item." << col->get_name() << ";" << endl;
        }
        ...
        if (this->has_pk_cond[table_name]) {
            ...
            if ((*conds)[0]->get_mode() == (*conds)[0]->Mode::JOIN) {
                ...
                rhs = (*conds)[0]->get_parameter()->get_name();
            }

            (*ofs) << tab << "\t\t\tif (c_" << (*conds)[0]->get_column()->get_name() << " > "
                   << rhs << " || ftell(handle->file) > handle->header->index_offset["
            ...
            (*ofs) << tab << "\t\t\tif (ftell(handle->file) > handle->header->index_offset["
                   << table_name << "_header_count]) {" << endl << tab <<
                   "\t\t\t\tfree(inserted);" << endl << tab <<
                   "\t\t\t\treturn;" << endl << tab <<
                   "\t\t\t}" << endl << tab <<
