...
void Environment::print(std::ofstream *ofs, std::ofstream *ofl) {
    ...
    (*ofs) << "void " << this->get_name() << "_open_write(const char* filename) {" << endl <<
           ...
           "\t\treturn;" << endl <<
           endl <<
           "\t" << this->get_name() << "_write_handle = malloc(sizeof(struct " << this->get_name() << "_handle));"
           << endl <<
    ...
    (*ofl) << "struct " << this->get_name() << "_handle* " << this->get_name() << "_open_read(const char* filename);"
           << endl;

    (*ofs) << "struct " << this->get_name() << "_handle* " << this->get_name() << "_open_read(const char* filename) {"
           << endl <<
           "\tFILE* file;" << endl <<
           "\tif (!(file = fopen(filename, \"rb\")))" << endl <<
           "\t\treturn NULL;" << endl <<
           endl <<
           "\tstruct " << this->get_name() << "_handle* handle = malloc(sizeof(struct " << this->get_name()
           << "_handle));" << endl <<
