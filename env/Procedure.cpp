...
void Procedure::print(ofstream *ofs, ofstream *ofl) {
    ...
    if (this->mode == Procedure::READ) {
        ...
        (*ofs) << "void " << name << "_add(struct " << name << "_out* iter, struct " << name << "_out_data* selected) {"
               << endl <<
    ...
    if (this->mode == Procedure::READ) {
        (*ofl) << "void " << name << "_init(struct " << name << "_out* iter, struct "
               << Environment::get_instance()->get_name() << "_handle* handle";
        (*ofs) << "void " << name << "_init(struct " << name << "_out* iter, struct "
               << Environment::get_instance()->get_name() << "_handle* handle";
        ...
        (*ofs) << "int " << name << "_next(struct " << name << "_out* iter) {" << endl <<
               ...
               "\t\tmemcpy(&iter->data, &(service->set[service->count]), sizeof(struct " << name << "_out_data));"
               << endl <<
               "\t\tservice->count++;" << endl <<
               "\t\treturn 1;" << endl <<
               "\t} else {" << endl <<
